...
def main():
    ...
    new_img1 = GLPF(old_img, 5)
    new_img2 = GLPF(old_img, 15)
    new_img3 = GLPF(old_img, 30)
    new_img4 = GLPF(old_img, 80)
    new_img5 = GLPF(old_img, 230)
...
def GLPF(img, r):
    ...
    for x in range(row):
        for y in range(col):
            ...
            f[x, y] = np.exp(-np.power(D, 2) / (2 * np.power(r, 2)))
